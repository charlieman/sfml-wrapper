...
pub const Event = union(Event.Type) {
    ...
    pub fn fromCSFML(event: sf.c.sfEvent) Self {
        return switch (event.type) {
            ...
            sf.c.sfEventType.sfEvtMouseWheelMoved, => .{ .mouseWheelScrolled = .{ .wheel = undefined, .delta = @intToFloat(f32, event.mouseWheel.delta), .pos = .{ .x = event.mouseWheel.x, .y = event.mouseWheel.y } } },
            sf.c.sfEventType.sfEvtMouseWheelScrolled => .{ .mouseWheelScrolled = .{ .wheel = @intToEnum(sf.window.mouse.Wheel, @enumToInt(event.mouseWheelScroll.wheel)), .delta = event.mouseWheelScroll.delta, .pos = .{ .x = event.mouseWheelScroll.x, .y = event.mouseWheelScroll.y } } },
